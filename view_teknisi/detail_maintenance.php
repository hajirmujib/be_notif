...
?>
          ...
          <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-4 order-1 order-md-2">
              ...
              <div class="project-info-box text-mute">
                ...
                <p class="text-secondary d-block"><b>Laporan Survey:</b> 
                  <?php if($lap_survey=="") {?>
                  ...
                  <?php }else{ ?>
                    <a href="download.php?filename=<?= $lap_survey ?>" id="download" target="_blank"><?php
                ...
                <p class="text-secondary d-block"><b>Surat Kerja:</b> <a href="download.php?filename=<?= $surat_kerja ?>" id="download" target="_blank"><?php
           ...
           <form method="POST" enctype="multipart/form-data">
            ...
            <div class="col-sm-4">
                <div class="card" style="width: 18rem;">
                  ...
                  <a href="delete_dokumentasi.php?id=<?= $row['id']; ?>&id_m=<?= $id ?>" class="btn btn-danger">Hapus</a>
                </div>
              </div>
            <?php }?>
              </div>
  ...
  if(isset($_POST['simpan'])){
    ...
    $newName=date('dmYHis').'_'.end($namafoto);

    $lokasi = $_FILES ['foto']['tmp_name'];
    $move=move_uploaded_file($lokasi,"../foto/".$newName);

    $queryDok=$conn->query("INSERT INTO dokumentasi VALUES(NULL,'$id','$newName','$keterangan')");
     if ($queryDok) {
  ...
  if(isset($_POST['upload1'])){
    ...
    $newName=date('D-M-Y H:i:s').'_'.'lap_survey'.'_'.$newNameUser.''.end($namafoto);

    $lokasi = $_FILES ['file1']['tmp_name'];
    $move=move_uploaded_file($lokasi,"../laporan/".$newName);


    $result=mysqli_query($conn,"UPDATE maintenance SET lap_survey='$newName' WHERE id='$id'");
    ...
    $namafoto=explode(".", $_FILES["file2"]["name"]);
    $newNameUser=strtok($instansi,' ');
    $newName=date('D-M-Y H:i:s').'_'.'laporan'.'_'.$newNameUser.''.end($namafoto);

    $lokasi = $_FILES ['file2']['tmp_name'];
    $move=move_uploaded_file($lokasi,"../laporan/".$newName);


    $result=mysqli_query($conn,"UPDATE maintenance SET laporan='$newName' WHERE id='$id'");
